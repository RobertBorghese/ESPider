...
modify_Game_Map = class {
	...
	startBoss2() {
		...
		SceneManager._scene._spriteset._tilemap.addPlayerBasedSprite(this._boss2Face);
	}

	updateBoss2() {
		this._boss2Timer++;

		if(this._boss2Timer > 120 && this._boss2Timer < 240) {
			this._boss2Face._baseY = -300 + (360 * Easing.easeOutCubic(((this._boss2Timer - 120) / 120)));
		} else if(this._boss2Timer >= 260 && this._boss2Timer <= 280) {
			const r = Easing.easeInCubic((this._boss2Timer - 260) / 20);
			this._boss2Face._baseY = 60 - (r * 40);
			this._boss2Face.setOpenness(-0.5 * r);
		} else if(this._boss2Timer >= 290 && this._boss2Timer <= 300) {
			const r = Easing.easeInCubic((this._boss2Timer - 290) / 10);
			this._boss2Face._baseY = 20 + (r * 90);
			this._boss2Face.setOpenness(-0.5 + (1 * r));
		} else if(this._boss2Timer >= 320 && this._boss2Timer <= 340) {
			const r = Easing.easeOutCubic((this._boss2Timer - 320) / 20);
			this._boss2Face._baseY = 110 - (50 * r)
			this._boss2Face.setOpenness(0.5 - (0.5 * r));
			this._boss2TempData = this._boss2Face._left.rotation;
		} else if(this._boss2Timer > 380) {
			...
			if(this._boss2Timer <= 400) {
				this._boss2Face.setOpenness(this._boss2TempData + ((targetOpenness - this._boss2TempData) * ((this._boss2Timer - 380) / 20)));
			} else {
		...
		if(this._boss2Timer === 300) {
			SceneManager._scene._spriteset.shake();
			ESPAudio.boss2Roar();
		}

		this._boss2Face.y = (this._boss2Face._baseY + (-80 * (Math.sin(this._boss2LeftLeg._time) * 0.3)));
	}
	...
	updateSwingAttack() {
		...
		if(this._attackTimer === 80) {
			if(!this._attackSprite) {
				...
				SceneManager._scene._spriteset._tilemap.addPlayerBasedSprite(this._attackSprite);
			}
		...
		} else if(this._attackTimer > 80 && this._attackTimer <= 100) {
			const r = Easing.easeOutCubic((this._attackTimer - 80) / 20);
			this._attackSprite.y = -800 + (900 * r);
		} else if(this._attackTimer > 100 && this._attackTimer <= 120) {
			const r = Easing.easeOutQuad((this._attackTimer - 100) / 20);

			this._attackSprite.rotation = 0.1 * r;
			this._attackSprite2.rotation = 0.2 * r;
			this._attackSprite3.rotation = 0.5 * r;

			this._attackSprite.y = 100 - (100 * r);
		} else if(this._attackTimer > (120 + START_WAIT) && this._attackTimer <= (120 + ATTACK_DURATION)) {
			const r = Easing.easeOutCubic((this._attackTimer - (120 + START_WAIT)) / (ATTACK_DURATION - START_WAIT));
			this._attackSprite.x = 400 + (500 * r);

			this._attackSprite.rotation = 0.1 - (0.3 * r);
			this._attackSprite2.rotation = 0.2 - (0.6 * r);
			this._attackSprite3.rotation = 0.5 - (1.1 * r);

		} else if(this._attackTimer > (120 + ATTACK_DURATION) && this._attackTimer <= (120 + ATTACK_DURATION + LEAVE_DURATION)) {
			const r = (this._attackTimer - (120 + ATTACK_DURATION)) / LEAVE_DURATION;
			this._attackSprite.y = 0 - (800 * r);
		} else if(this._attackTimer === (120 + ATTACK_DURATION + LEAVE_DURATION + 1)) {
			this._attackSprite.visible = false;
			this._boss2LeftLeg.goDown(280);
		} else if(this._attackTimer === (120 + ATTACK_DURATION + LEAVE_DURATION + 120)) {
			this._currentAttack = 0;
		}
	}
}
