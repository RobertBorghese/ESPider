...
class ESPButton extends Sprite_Clickable {
	constructor(width, height, text, colorNormal, colorHover, colorClickFlash, colorUnderline, callback) {
		...
		this._graphicsBack.beginFill(this._colorNormal);
		this._graphicsBack.drawRect(offset, height - offset, 2, offset);
		this._graphicsBack.drawRect(offset, height - 2, width - offset, 2);
		this._graphicsBack.drawRect(width - 2, offset, 2, height - offset);
		this._graphicsBack.drawRect(width - 2, offset, 2, height - offset);
		this._graphicsBack.drawRect(width - offset, offset, offset, 2);
	...
	updateGraphics() {
		const easing = (this._espHovered ? Easing.easeOutCubic : Easing.easeOutCubic);
		const ratio = easing(this._espClicked ? 1 : this._state.clamp(0, 1));
		const offset = this.__espOffset;
		...
		this._graphicsFront.drawRect(0, 0, this.__espWidth - offset, this.__espHeight - offset);
		this._graphicsFront.endFill();
		const ww = (this.__espWidth - offset) * ratio;
		const x = ((this.__espWidth - ww) / 2) - (offset / 2);
		const x2 = x + ww;
		if(!this._espClicked) {
			this._graphicsFront.beginFill(this._colorHover);
			this._graphicsFront.drawRect(x, 0, ww, this.__espHeight - offset);
			this._graphicsFront.endFill();
		}
		...
		if(ratio > 0) {
			...
			this._graphicsFront.drawRect(x2, 0, 2, this.__espHeight - offset);
			this._graphicsFront.endFill();
		}

		if(this._style) {
			this._text.x = (this.__espWidth - this._text.width) / 2;
			this._style.letterSpacing = (this._maxLetterSpacing * ratio) + 1;
			this._text.style = this._style;
		}
	}

	onPress() {
		this._graphicsFront.x = this._graphicsFront.y = 6;
	}
